...
use crate::mp4box::{
    av01::{Av01Box, Av1CBox},
    avc1::Avc1Box,
    co64::Co64Box,
    ctts::CttsBox,
    ctts::CttsEntry,
    hvc1::{Hvc1Box, HvcCBox},
    mp4a::Mp4aBox,
    smhd::SmhdBox,
    stco::StcoBox,
    stsc::StscEntry,
...
impl From<MediaConfig> for TrackConfig {
    fn from(media_conf: MediaConfig) -> Self {
        match media_conf {
            ...
            MediaConfig::Av1Config(av1_conf) => Self::from(av1_conf),
...
impl Mp4Track {
    ...
    pub fn trak(&self) -> &TrakBox {
        &self.trak
    }

    pub fn track_name(&self) -> &str {
        &self.trak.mdia.hdlr.name
    }
    ...
    pub fn keyframe_timestamps_seconds(&self) -> Vec<f64> {
        let timescale = self.timescale() as f64;
        if timescale <= 0.0 {
            return Vec::new();
        }
        if let Some(ref stss) = self.trak.mdia.minf.stbl.stss {
            stss.entries
                ...
                .filter_map(|&sample_id| {
                    self.sample_time(sample_id)
                        .ok()
                        .map(|(start_time, _)| (start_time as f64) / timescale)
                })
                .collect()
        } else {
    ...
    pub fn media_type(&self) -> Result<MediaType> {
        if self.trak.mdia.minf.stbl.stsd.avc1.is_some() {
            ...
            || self.trak.mdia.minf.stbl.stsd.hev1.is_some()
        {
            Ok(MediaType::H265)
    ...
    pub fn width(&self) -> u16 {
        if let Some(ref avc1) = self.trak.mdia.minf.stbl.stsd.avc1 {
            ...
            hev1.width
        } else if let Some(ref av01) = self.trak.mdia.minf.stbl.stsd.av01 {
            av01.width
        } else {
    ...
    pub fn av1c(&self) -> Option<&Av1CBox> {
        if let Some(ref av01) = self.trak.mdia.minf.stbl.stsd.av01 {
            Some(&av01.av1c)
        } else {
            None
        }
    ...
    pub fn sample_pts(&self, sample_id: u32) -> Result<u64> {
        let (dts, _) = self.sample_time(sample_id)?;
        let offset = self.sample_rendering_offset(sample_id);
        Ok((dts as i64 + offset as i64).max(0) as u64)
    }
...
impl Mp4TrackWriter {
    pub(crate) fn new(track_id: u32, config: &TrackConfig) -> Result<Self> {
        ...
        if let Some(ref name) = config.track_name {
            trak.mdia.hdlr.name = name.clone();
        }
        ...
        match config.media_conf {
            ...
            MediaConfig::Av1Config(ref av1_config) => {
                ...
                let av01 = Av01Box::new(av1_config)?;
                trak.mdia.minf.stbl.stsd.av01 = Some(av01);
            }
    ...
    pub(crate) fn from_trak_sample_description(track_id: u32, source: &TrakBox) -> Self {
        let mut trak = TrakBox::default();
        trak.tkhd = source.tkhd.clone();
        trak.tkhd.track_id = track_id;
        trak.tkhd.duration = 0;
        trak.edts = source.edts.clone();
        trak.mdia.mdhd = source.mdia.mdhd.clone();
        trak.mdia.mdhd.duration = 0;
        trak.mdia.hdlr = source.mdia.hdlr.clone();
        trak.mdia.minf.vmhd = source.mdia.minf.vmhd.clone();
        trak.mdia.minf.smhd = source.mdia.minf.smhd.clone();
        trak.mdia.minf.dinf = source.mdia.minf.dinf.clone();
        trak.mdia.minf.stbl.stsd = source.mdia.minf.stbl.stsd.clone();
        trak.mdia.minf.stbl.co64 = Some(Co64Box::default());
        ...
        Mp4TrackWriter {
            ...
            sample_id: 1,
            fixed_sample_size: 0,
            is_fixed_sample_size: false,
            chunk_samples: 0,
            chunk_duration: 0,
            chunk_buffer: BytesMut::new(),
            samples_per_chunk: 0,
            duration_per_chunk: source.mdia.mdhd.timescale,
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_track_config_from_hevc_writes_reads_hvc1() {
        let hevc_conf = sample_hevc_config();
        let track_config = TrackConfig::from(hevc_conf.clone());
        assert_eq!(track_config.track_type, TrackType::Video);

        let mp4_config = Mp4Config {
            major_brand: "mp42".parse().unwrap(),
            minor_version: 0,
            compatible_brands: vec!["mp42".parse().unwrap(), "isom".parse().unwrap()],
            timescale: 1000,
        };
        ...
        let sample = Mp4Sample {
            ...
            rendering_offset: 0,
            is_sync: true,
            bytes: Bytes::from(vec![0x00, 0x00, 0x00, 0x04, 0x26, 0x01, 0xaf, 0x01]),
        };
        ...
        assert_eq!(track.media_type().unwrap(), MediaType::H265);
        assert_eq!(track.box_type().unwrap(), FourCC::from(BoxType::Hvc1Box));
        assert_eq!(track.box_type().unwrap().value, *b"hvc1");
        assert_eq!(track.width(), 1920);
        assert_eq!(track.height(), 1080);

        let hvcc = track.hvcc().expect("hvcc should be present on hvc1 track");
        ...
        assert_eq!(hvcc.arrays[0].nal_unit_type, 32); // VPS
        assert_eq!(hvcc.arrays[0].nal_units[0], hevc_conf.vps[0]);
        assert_eq!(hvcc.arrays[1].nal_unit_type, 33); // SPS
        assert_eq!(hvcc.arrays[1].nal_units[0], hevc_conf.sps[0]);
        assert_eq!(hvcc.arrays[2].nal_unit_type, 34); // PPS
        assert_eq!(hvcc.arrays[2].nal_units[0], hevc_conf.pps[0]);
    }

    #[test]
    fn test_hev1_read_compatibility() {
        // Construct a trak with hev1 entry in stsd and verify reader returns H265 & hev1 FourCC
        let mut stsd = StsdBox::default();
        let hevc_conf = sample_hevc_config();
        let hev1 = Hev1Box::new(&hevc_conf).unwrap();
        stsd.hev1 = Some(hev1);

        let mut trak = TrakBox::default();
        trak.tkhd.track_id = 1;
        trak.tkhd.set_width(1280);
        trak.tkhd.set_height(720);
        trak.mdia.hdlr.handler_type = FourCC::from(TrackType::Video);
        trak.mdia.minf.stbl.stsd = stsd;

        let track = Mp4Track::from(&trak);
        assert_eq!(track.media_type().unwrap(), MediaType::H265);
        assert_eq!(track.box_type().unwrap(), FourCC::from(BoxType::Hev1Box));
        assert_eq!(track.box_type().unwrap().value, *b"hev1");
        assert_eq!(track.width(), 1920);
        assert_eq!(track.height(), 1080);
        assert!(track.hvcc().is_some());
    }
    ...
    #[test]
    fn test_track_config_from_av1_writes_reads_av01() {
        let av1_conf = sample_av1_config();
        let track_config = TrackConfig::from(av1_conf.clone());
        assert_eq!(track_config.track_type, TrackType::Video);
        ...
        // Sample with valid size-prefixed OBUs (e.g. Frame OBU: type 6 -> (6 << 3) | 2 = 0x32, size 2)
        let sample = Mp4Sample {
            start_time: 0,
            duration: 100,
            rendering_offset: 0,
            is_sync: true,
            bytes: Bytes::from(vec![0x32, 0x02, 0xAA, 0xBB]),
        };
        writer.write_sample(1, &sample).unwrap();
        writer.write_end().unwrap();
        ...
        assert_eq!(track.track_type().unwrap(), TrackType::Video);
        assert_eq!(track.media_type().unwrap(), MediaType::AV1);
        assert_eq!(track.box_type().unwrap(), FourCC::from(BoxType::Av01Box));
        assert_eq!(track.box_type().unwrap().value, *b"av01");
        assert_eq!(track.width(), 1920);
        assert_eq!(track.height(), 1080);

        let av1c = track.av1c().expect("av1c should be present on av01 track");
        ...
        assert_eq!(av1c.initial_presentation_delay_minus_one, Some(2));
        assert_eq!(av1c.config_obus, av1_conf.config_obus);
    }
}
